...
@Component({
  selector: 'app-producto-actualizar-con-imagen',
  templateUrl: './producto-actualizar-con-imagen.component.html',
  styleUrls: ['./producto-actualizar-con-imagen.component.css']
})
export class ProductoActualizarConImagenComponent {
  ...
  public guardarFoto(formularito: NgForm): void {
    ...
    this.miSuscripcion = this.crearProducto
      .actualizarImagen(nuevoProducto)
      .pipe(
        map((resultado) => {
          mostrarMensaje('success', 'Foto actualizada', 'Actualizado', this.toastr);
        ...
        catchError((err) => {
          mostrarMensaje(
            'error',
            'error al actualizar la producto',
            'Error',
            this.toastr
          );
          formularito.reset();
